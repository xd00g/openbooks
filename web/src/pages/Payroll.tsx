          ...
          <div className="grid grid-cols-2 gap-2">
            {([
              ...
              <input key={f} value={emp[f]} onChange={(e) => setEmp({ ...emp, [f]: e.target.value })}
                placeholder={label} className="rounded-md border border-rule px-2 py-1 text-sm" />
            ))}
            <select value={emp.payType} onChange={(e) => setEmp({ ...emp, payType: e.target.value })} className="rounded-md border border-rule px-2 py-1 text-sm">
              <option value="hourly">Hourly</option>
              <option value="salary">Salary (annual)</option>
            </select>
            <label className="text-xs text-muted">Hire date<input type="date" value={emp.hireDate} onChange={(e) => setEmp({ ...emp, hireDate: e.target.value })} className="mt-1 w-full rounded-md border border-rule px-2 py-1 text-sm" /></label>
          </div>
          <div className="mt-2 flex items-center justify-between">
            <span className="text-xs text-muted">{(employees.data ?? []).length} employees</span>
            <Button onClick={() => addEmp.mutate()} disabled={!emp.firstName || !emp.lastName}>Add employee</Button>
          </div>
        </Card>
        <Card title="Pay period">
          <div className="grid grid-cols-3 gap-2 text-sm">
            <label className="text-xs text-muted">Pay date<input type="date" value={dates.payDate} onChange={(e) => setDates({ ...dates, payDate: e.target.value })} className="mt-1 w-full rounded-md border border-rule px-2 py-1" /></label>
            <label className="text-xs text-muted">Start<input type="date" value={dates.periodStart} onChange={(e) => setDates({ ...dates, periodStart: e.target.value })} className="mt-1 w-full rounded-md border border-rule px-2 py-1" /></label>
            <label className="text-xs text-muted">End<input type="date" value={dates.periodEnd} onChange={(e) => setDates({ ...dates, periodEnd: e.target.value })} className="mt-1 w-full rounded-md border border-rule px-2 py-1" /></label>
          </div>
        </Card>
      </div>
        ...
        <Table head={['Employee', 'Type', 'Hours', 'Gross', 'EE taxes', 'ER taxes', 'Deductions']}>
          {(employees.data ?? []).map((e: any) => (
            <tr key={e.id}>
              ...
              <td className="px-4 py-2 text-xs capitalize text-muted">{e.payType ?? 'hourly'}{e.payRate ? ` @ ${money(e.payRate)}` : ''}</td>
              {(['hours', 'gross', 'employeeTaxes', 'employerTaxes', 'deductions'] as const).map((f) => (
                <td key={f} className="px-4 py-1">
                  <input value={amts[e.id]?.[f] ?? ''} onChange={(ev) => setAmt(e.id, f, ev.target.value)}
                    ...
                    className="w-20 rounded-md border border-rule px-2 py-1 text-sm text-right disabled:bg-paper" />
                </td>
              ))}
            </tr>
          ))}
          {(employees.data ?? []).length === 0 && <tr><td colSpan={7} className="px-4 py-6 text-center text-sm text-muted">Add an employee first.</td></tr>}
        </Table>
        ...
        <Table head={['Pay date', 'Status', 'Gross', 'Net', '']}>
          {(runs.data ?? []).map((r: any) => (
            <tr key={r.id}>
              ...
              <td className="px-4 py-2 capitalize text-muted">{r.status}</td>
          ...
          {(runs.data ?? []).length === 0 && <tr><td colSpan={5} className="px-4 py-6 text-center text-sm text-muted">No runs yet.</td></tr>}
        </Table>
      </div>
    </Page>
  );
